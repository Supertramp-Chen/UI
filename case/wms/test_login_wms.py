...
def read_csv_file(file_path):
    ...
    但 pytest.mark.parametrize 在解析时会把所有 yield 生成的数据取出来。"""
    with open(file_path, 'r', newline='') as file:
        """ csv.DictReader(file) 逐行读取 CSV 文件，并转换成字典
         这是一个迭代器对象，把每次读取出来的数据都放到字典里面存起来，
         下面用一个for循环一次一次的去读取字典里面的数据，
         确保不会一次性将所有的数据读取到内存中。"""
        reader = csv.DictReader(file)
        """如果下面没有生成器，那么这里直接全部数据都遍历一遍，
        如果有生成器就会卡住，一个一个来，接收到next方法才会读取下一行。"""
        for row in reader:
