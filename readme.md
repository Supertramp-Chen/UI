...
- [ ] 第二阶段 收集测试用例 这时候加载测试类 
  ...
  用@pytest.mark.parametrize修饰测试用例 里面写上读取csv文件的函数 在pytest收集测试用例的时候会自动执行 根据里面的数据生成所有的测试用例 

  读取csv的函数是用csv.DictRead来一行行读取 然后用yield控制一行行读取csv里面的数据 
...
- [ ] 第三阶段 解析fixture装饰器 执行前置操作 按scope顺序session model class function
  ...
  测试类里装饰器修饰的函数把dirvers作为参数 自动获得driver实例
