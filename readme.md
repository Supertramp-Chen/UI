...
- [ ] 第二阶段 收集测试用例 这时候加载测试类 
  ...
  要先用iter函数变成选代器对象再用next方法遍历其中的每一个数据，
  ...
  可迭代对象一般是能直接用于for循环遍历的数据对象，比如：常见容器类型数据str，list，tuple，set，dict等都是可迭代对象，还有一些函数的返回值如range（函数的返回值也是一个迭代的对象
  迭代器和可迭代对象很像，是可以被next函数调用并不断返回下一个值的对象
  可迭代对象一次性生成全部值，
  迭代器每次被next函数调用只生成一个值，不会一次性全部生成
  生成器是一种特殊的迭代器，通过yield关键字来创建，生成器运行时每次遇到yield时，函数会暂停并保存当前运行信息，下次执行next时从暂停位置继续运行
  
  
  iter__()：告诉 Python，“这是一个可迭代的对象，你可以用它来获取元素。” 该方法返回一个迭代器对象
  
  		next__()：告诉python下一个元素是什么，每次调用时返回一个新元素，直到没有元素时抛出一个StopIteration异常
  		
  迭代器就是用来遍历可迭代对象
  可迭代对象只实现iter方法：列表、字符串、字典、集合等，
  	range源码：实现了iter方法 所以是可迭代对象
  	list内部：实现了iter方法 所以是可迭代对象
  迭代器（如通过iter函数创建的对象）实现iter和next方法，可以直接使用 next获取下一个元素（也自动是 Iterable）
  	每次调用 next方法会返回可迭代对象中的下一个元素，直到所有元素被遍历完
  	如果继续调用next方法，将会触发 StopIteration 异常
  可迭代对象转化为迭代器：
  	iter是一个“工厂”，它生产出一个迭代器对象。
  	这个迭代器就像一个指针，指向列表的当前元素，每次调用 next 时，它都会移动到下一个元素，直到列表结束
  ```
